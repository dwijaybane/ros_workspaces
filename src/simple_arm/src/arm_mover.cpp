...
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2)
{
    ...
    // we are creating topics that we can give value later through launch
    // Lets just assume they exist and get value from it
    // Parameters are private namespace topics
    ...
    // Check if joint 1 falls in the safe zone, otherwise clamp it (failiur progam)
